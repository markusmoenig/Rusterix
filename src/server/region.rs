...
impl RegionInstance {
    ...
    pub fn run(self) {
        ...
        std::thread::spawn(move || {
            ...
            *CURR_ITEMID.borrow_mut() = None;
            ...
            send_log_message(
                0,
                format!("{}: Startup with {} errors.", name, *ERROR_COUNT.borrow(),),
            );
    ...
    fn create_sleep_switch_action(&self, minutes: u32, switchback: EntityAction) -> EntityAction {
        with_regionctx(self.id, |ctx| {
            let tick = ctx.ticks + (minutes as i64 * ctx.ticks) as i64;
